import { Highlighter, LanguageInput, getSingletonHighlighter } from 'shiki'
...
const groovy = {
  ...
  repository: {
    ...
    methods: {
      applyEndPatternLast: 1,
      begin:
        '(?:(?<=;|^|{)(?=\\s*(?:(?:private|protected|public|native|synchronized|abstract|threadsafe|transient|static|final)|(?:def)|(?:(?:(?:void|boolean|byte|char|short|int|float|long|double)|(?:@?(?:[a-zA-Z]\\w*\\.)*[A-Z]+\\w*))[\\[\\]]*(?:<.*>)?))\\s+([^=]+\\s+)?\\w+\\s*\\())',
      end: '}|(?=[^{])',
    ...
    numbers: {
      patterns: [
        {
          match:
            '((0(x|X)[0-9a-fA-F]*)|(\\+|-)?\\b((\\d+\\.?\\d*)|(\\.\\d+))((e|E)(\\+|-)?\\d+)?)([LlFfUuDdg]|UL|ul)?\\b',
    ...
    'support-functions': {
      patterns: [
        ...
        {
          match:
            '\\b(?:shouldFail|fail(?:NotEquals)?|ass(?:ume|ert(?:S(?:cript|ame)|N(?:ot(?:Same|Null)|ull)|Contains|T(?:hat|oString|rue)|Inspect|Equals|False|Length|ArrayEquals)))\\b',
    ...
    variables: {
      ...
      patterns: [
        {
          begin:
            '(?:(?=(?:(?:private|protected|public|native|synchronized|abstract|threadsafe|transient|static|final)|(?:def)|(?:void|boolean|byte|char|short|int|float|long|double)|(?:(?:[a-z]\\w*\\.)*[A-Z]+\\w*))\\s+[\\w\\d_<>\\[\\],\\s]+(?:=|$)))',
          end: ';|$',
...
const allThemes = ['github-dark', 'nord']
...
export const getSharedHighlighter = async (): Promise<Highlighter> => {
  if (!sharedHighlighterPromise) {
    sharedHighlighterPromise = getSingletonHighlighter({
      themes: allThemes,
      langs: allLanguages as LanguageInput[],
    })
  }
