export const metadata = {
  title: 'Comprehensive Guide to Two Heaps Technique in JavaScript',
  description: 'Maintain two heaps to solve interval and median problems. Understand the two heaps technique, performance considerations, and practical examples.',
}
...
Imagine you have a stream of numbers, and you need to find the median after each insertion. Using two heaps allows you to maintain the median efficiently by balancing the largest elements of the lower half (Max Heap) and the smallest elements of the upper half (Min Heap).
...
class MaxHeap {
    ...
    extractMax(): number {
        if (this.size() === 1) return this.heap.pop() as number;
        const max = this.heap[0];
...
// Add a remove method to the MaxHeap and MinHeap classes for completeness
MaxHeap.prototype.remove = function(value: number): void {
    const index = this.heap.indexOf(value);
    if (index === -1) return;
    if (index === this.heap.length - 1) {
        this.heap.pop();
    } else {
        this.heap[index] = this.heap.pop() as number;
        this._heapifyDown(index);
        this._heapifyUp(index);
    }
};

MinHeap.prototype.remove = function(value: number): void {
    const index = this.heap.indexOf(value);
    if (index === -1) return;
    if (index === this.heap.length - 1) {
        this.heap.pop();
