export const metadata = {
  title: 'Comprehensive Guide to Network Flow Algorithms in JavaScript',
  description: 'Solve flow problems in networks, such as maximum flow and minimum cut. Understand network flow algorithms, performance considerations, and practical examples.',
}
...
- **Flow Network:** A directed graph where each edge has a capacity and each vertex, except the source and sink, follows flow conservation.
- **Residual Graph:** A graph showing remaining capacities after considering current flow.
- **Augmenting Path:** A path from the source to the sink in the residual graph where the flow can be increased.
...
class CapacityScaling {
    ...
    bfs(source: number, sink: number, parent: number[], minCapacity: number): boolean {
        const visited = Array(this.V).fill(false);
        const queue: number[] = [source];
        visited[source] = true;
