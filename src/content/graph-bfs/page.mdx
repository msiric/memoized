...
BFS traversal starting from vertex `A` will visit vertices in the following order: `A -> B -> C -> D -> E -> F`.
...
```typescript
function bfsShortestPath(graph: { [key: string]: string[] }, start: string): { [key: string]: number } {
    let distances: { [key: string]: number } = {};
    let queue: string[] = [start];
    distances[start] = 0;

    while (queue.length > 0) {
        let vertex = queue.shift() as string;
...
```typescript
function findConnectedComponents(graph: { [key: string]: string[] }): string[][] {
...
```typescript
function containsCycle(graph: { [key: string]: string[] }): boolean {
    let visited = new Set<string>();

    for (let vertex in graph) {
        if (!visited.has(vertex)) {
            let queue: { vertex: string; parent: string | null }[] = [{ vertex, parent: null }];

            while (queue.length > 0) {
                let { vertex: v, parent } = queue.shift() as { vertex: string; parent: string | null };
                if (!visited.has(v)) {
                    visited.add(v);
