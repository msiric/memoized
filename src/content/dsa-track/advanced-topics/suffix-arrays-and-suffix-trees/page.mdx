export const metadata = {
  title: 'Comprehensive Guide to Suffix Arrays and Suffix Trees in JavaScript',
  description: 'Advanced data structures for efficient string processing. Understand suffix arrays and suffix trees, performance considerations, and practical examples.',
}
...
- **Suffix Array:** A sorted array of the starting positions of all suffixes.
  ```
  String: "banana"
  ...
  ```
- **Suffix Tree:** A tree representing all suffixes where each path from the root represents a suffix.
...
function buildSuffixArray(s: string): number[] {
    ...
    const suffixArray: number[] = suffixes.map(suffix => suffix.index);
    return suffixArray;
}
...
class SuffixTree {
    ...
    extendSuffixTree(pos: number, activeNode: SuffixTreeNode, activeEdge: number, activeLength: number, remainingSuffixCount: number, leafEnd: number) {
        leafEnd = pos;
        remainingSuffixCount++;
...
function buildLCPArray(s: string, suffixArray: number[]): number[] {
    ...
    const lcp: number[] = Array(n).fill(0);
...
// Example usage:
const s1: string = "banana";
const s2: string = "anana";
console.log(longestCommonSubstring(s1, s2));  // Output: "anana"
```
