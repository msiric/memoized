export const metadata = {
  title: 'Comprehensive Guide to Maximum Flow in JavaScript',
  description: 'Calculate the maximum possible flow in a flow network. Understand maximum flow algorithms, performance considerations, and practical examples.',
}

# **Maximum Flow**

The Maximum Flow problem involves finding the maximum flow from a source vertex to a sink vertex in a flow network. A flow network is a directed graph where each edge has a capacity, and the goal is to maximize the flow from the source to the sink while respecting the capacities.

## **Concept and Use Cases**

**Definition:**
The Maximum Flow problem seeks to determine the maximum amount of flow that can be sent from a source vertex to a sink vertex in a flow network, such that the flow on each edge does not exceed its capacity.
...
Consider a flow network with vertices `{S, A, B, T}` and edges with capacities:
```
S --10--> A --4--> B --10--> T
S --10--> B
A --2--> T
```
The maximum flow from `S` to `T` is 12.
...
## **Maximum Flow Algorithms and Methods**
...
class FordFulkerson {
    private V: number;
    private graph: Array<[number, number][]>;
...
class CapacityScaling {
    ...
    private bfs(source: number, sink: number, parent: number[], minCapacity: number): boolean {
        const visited = Array(this.V).fill(false);
        const queue = [source];
        visited[source] = true;
    ...
    capacityScaling(source: number, sink: number): number {
        let maxFlow = 0;
        const parent = Array(this.V).fill(-1);
        let maxCapacity = Math.max(...this.graph.flat().map(([_, capacity]) => capacity));
...
class Dinic {
    private V: number;
    private graph: Array<[number, number, number][]>;
    ...
    private dfs(u: number, sink: number, flow: number): number {
        if (u === sink) return flow;
    ...
    dinic(source: number, sink: number): number {
        let maxFlow = 0;
...
By mastering Maximum Flow algorithms and understanding their intricacies, you will be well-equipped to handle a variety of flow network problems. Regular practice and a solid grasp of advanced topics will deepen your understanding and improve your problem-solving skills.