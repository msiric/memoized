export const metadata = {
  title: 'Comprehensive Guide to Union Find in JavaScript',
  description: 'Detect cycles and manage dynamic connectivity in graphs. Understand union find, performance considerations, and practical examples.',
}
...
Consider a set of elements `{0, 1, 2, 3, 4, 5}` and a series of union operations:

```
Initial sets: {0}, {1}, {2}, {3}, {4}, {5}
Union(1, 2): {0}, {1, 2}, {3}, {4}, {5}
Union(2, 3): {0}, {1, 2, 3}, {4}, {5}
Union(4, 5): {0}, {1, 2, 3}, {4, 5}
```

After these operations, `find(1)` will return the representative of the set containing `1`, which could be `1` or `2` or `3` depending on the implementation details.
...
class KruskalMST {
    ...
    kruskalMST(): [number, number, number][] {
        this.edges.sort((a, b) => a[2] - b[2]);
        const parent: number[] = [];
        const rank: number[] = [];
...
class DynamicConnectivity {
    ...
    addConnection(u: number, v: number): void {
        this.uf.union(u, v);
    }
