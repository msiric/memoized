...
```typescript
function dfsCycleDetection(graph: { [key: string]: string[] }): boolean {
    let visited = new Set<string>();
    let recStack = new Set<string>();
...
function findConnectedComponents(graph: { [key: string]: string[] }): string[][] {
    ...
    function dfs(vertex: string, component: string[]) {
        visited.add(vertex);
        component.push(vertex);
...
```typescript
function findAllPaths(graph: { [key: string]: string[] }, start: string, end: string): string[][] {
    let result: string[][] = [];
