...
Consider the intervals `[[1, 3], [2, 6], [8, 10], [15, 18]]`. After merging, the intervals will be `[[1, 6], [8, 10], [15, 18]]`.
...
// Example usage:
const intervals: number[][] = [[1, 3], [6, 9]];
const newInterval: number[] = [2, 5];
console.log(insertInterval(intervals, newInterval));  // Output: [[1, 5], [6, 9]]
```
...
// Example usage:
const A: number[][] = [[0, 2], [5, 10], [13, 23], [24, 25]];
const B: number[][] = [[1, 5], [8, 12], [15, 24], [25, 26]];
console.log(intervalIntersection(A, B));  // Output: [[1, 2], [5, 5], [8, 10], [15, 23], [24, 24], [25, 25]]
```
