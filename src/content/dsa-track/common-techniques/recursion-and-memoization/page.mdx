...
Consider the Fibonacci sequence. Without memoization, the function recalculates values multiple times:
```
fib(5)
|- fib(4)
|  |- fib(3)
...
|- fib(3)
   ...
   |- fib(1)
```
With memoization, previously computed results are reused, reducing redundant calculations.
...
```typescript
function factorialMemo(n: number, memo: Record<number, number> = {}): number {
    if (n === 0) return 1;
    if (memo[n]) return memo[n];
    memo[n] = n * factorialMemo(n - 1, memo);
    return memo[n];
...
```typescript
function lcs(x: string, y: string, m: number, n: number): number {
    if (m === 0 || n === 0) return 0;
    if (x[m - 1] === y[n - 1]) return 1 + lcs(x, y, m - 1, n - 1);
    return Math.max(lcs(x, y, m, n - 1), lcs(x, y, m - 1, n));
}
...
```typescript
function lcsMemo(x: string, y: string, m: number, n: number, memo: Record<string, number> = {}): number {
    const key = `${m},${n}`;
    if (memo[key]) return memo[key];
    if (m === 0 || n === 0) return 0;
    if (x[m - 1] === y[n - 1]) {
...
```typescript
function knapsackMemo(W: number, weights: number[], values: number[], n: number, memo: Record<string, number> = {}): number {
    const key = `${n},${W}`;
    if (memo[key]) return memo[key];
    if (n === 0 || W === 0) return 0;
    if (weights[n - 1] > W) {
...
```typescript
function tailRecFactorial(n: number, acc: number = 1): number {
    if (n === 0) return acc;
    return tailRecFactorial(n - 1, n * acc);
}
