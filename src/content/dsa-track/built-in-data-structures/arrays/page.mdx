export const metadata = {
  title: 'Comprehensive Guide to JavaScript Arrays',
  description: 'Learn about array methods and how to manipulate arrays. Understand array creation, performance considerations, and practical examples.',
}
...
**Definition:**
A JavaScript array is an ordered, mutable collection of values, where each value is identified by an index. Arrays can grow and shrink dynamically, and they come with a rich set of methods for performing various operations.
...
let mixedArray: (string | number | Record<string, number> | number[])[] = [1, 'two', {three: 3}, [4, 5]];
let arrayWithConstructor: unknown[] = new Array(10);  // Creates an array with 10 empty slots
```
...
// Map
let doubled: number[] = arr.map(x => (typeof x === 'number' ? x * 2 : x));
console.log(doubled);  // Output: [2, 4, 6, 8, 10]

// Filter
let evens: number[] = arr.filter(x => (typeof x === 'number' && x % 2 === 0));
console.log(evens);  // Output: [2, 4]

// Reduce
let sum: number = arr.reduce((acc, x) => (typeof x === 'number' ? acc + x : acc), 0);
console.log(sum);  // Output: 15

// Find
let firstEven: number | undefined = arr.find(x => (typeof x === 'number' && x % 2 === 0));
console.log(firstEven);  // Output: 2

// Some
let hasEven: boolean = arr.some(x => (typeof x === 'number' && x % 2 === 0));
console.log(hasEven);  // Output: true

// Every
let allEven: boolean = arr.every(x => (typeof x === 'number' && x % 2 === 0));
console.log(allEven);  // Output: false

// Slice
let subArray: (number | string)[] = arr.slice(1, 3);
console.log(subArray);  // Output: [2, 3]
...
// Concat
let arr2: number[] = [6, 7, 8];
let concatenated: (number | string)[] = arr.concat(arr2);
console.log(concatenated);  // Output: [1, 2, 'a', 'b', 4, 5, 6, 7, 8]
```
...
- **Using `Array.from` and `Array.of`:**
  ...
  **Example:**
  ...
  let arr2: number[] = Array.of(1, 2, 3);
  console.log(arr2);  // Output: [1, 2, 3]
  ```
...
- **Flattening Arrays:**
  ...
  **Example:**
  ...
  ```typescript
  let nestedArr: (number | (number | number[])[])[] = [1, [2, [3, 4], 5]];
  let flatArr: number[] = nestedArr.flat(2) as number[];
  console.log(flatArr);  // Output: [1, 2, 3, 4, 5]
  ```
...
- **Removing Duplicates:**
  ...
  **Example:**
  ...
  ```typescript
  let arr: number[] = [1, 2, 2, 3, 3, 4];
  let uniqueArr: number[] = Array.from(new Set(arr));
  console.log(uniqueArr);  // Output: [1, 2, 3, 4]
  ```
...
- **Array-Like Objects:**
  ...
  **Example:**
  ...
  ```typescript
  function example(...args: number[]): void {
      let argsArray: number[] = Array.from(args);
...
let int32View: Int32Array = new Int32Array(buffer);
int32View[0] = 42;
console.log(int32View);  // Output: Int32Array [ 42, 0, 0, 0 ]
```
...
let result: number[] = arr.flatMap(x => [x, x * 2]);
console.log(result);  // Output: [1, 2, 2, 4, 3, 6, 4, 8]

// ReduceRight
let arr2: number[] = [1, 2, 3, 4];
let result2: number = arr2.reduceRight((acc, x) => acc - x, 0);
console.log(result2);  // Output: -10
...
```typescript
let arr: number[] = [10, 20, 30, 40, 50];
let found: number | undefined = arr.find(x => x > 25);
console.log(found);  // Output: 30
```
