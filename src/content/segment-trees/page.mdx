export const metadata = {
  title: 'Comprehensive Guide to Segment Trees in JavaScript',
  description: 'Efficiently perform range queries on arrays. Understand segment trees, performance considerations, and practical examples.',
}
...
Each node represents an interval `[start, end]`, and each leaf node represents a single element of the array.
...
class SegmentTree {
    ...
    update(index: number, value: number, node = 0, start = 0, end = this.n - 1) {
        if (start === end) {
            this.tree[node] = value;
        } else {
            let mid = Math.floor((start + end) / 2);
    ...
    query(L: number, R: number, node = 0, start = 0, end = this.n - 1): number {
        if (R < start || L > end) {
            return 0;
        }
        if (L <= start && end <= R) {
...
class LazySegmentTree {
    ...
    updateRange(L: number, R: number, value: number, node = 0, start = 0, end = this.n - 1) {
        if (this.lazy[node] !== 0) {
            this.tree[node] += (end - start + 1) * this.lazy[node];
            if (start !== end) {
                this.lazy[2 * node + 1] += this.lazy[node];
    ...
    queryRange(L: number, R: number, node = 0, start = 0, end = this.n - 1): number {
        if (this.lazy[node] !== 0) {
            this.tree[node] += (end - start + 1) * this.lazy[node];
            if (start !== end) {
                this.lazy[2 * node + 1] += this.lazy[node];
...
class PersistentSegmentTree {
  ...
  private build(arr: number[]) {
    const buildRecursive = (start: number, end: number): SegmentTreeNode => {
      if (start === end) {
        return { sum: arr[start], left: null, right: null };
      }
      let mid = Math.floor((start + end) / 2);
  ...
  private update(root: SegmentTreeNode, index: number, value: number, start: number, end: number): SegmentTreeNode {
    if (start === end) {
      return { sum: value, left: null, right: null };
    }
    let mid = Math.floor((start + end) / 2);
    ...
    let right = root.right;
    if (index <= mid) {
      left = this.update(root.left as SegmentTreeNode, index, value, start, mid);
    } else {
      right = this.update(root.right as SegmentTreeNode, index, value, mid + 1, end);
    }
    return { sum: (left as SegmentTreeNode).sum + (right as SegmentTreeNode).sum, left, right };
  }

  private query(root: SegmentTreeNode, L: number, R: number, start: number, end: number): number {
    if (R < start || L > end) {
      return 0;
    }
    if (L <= start && end <= R) {
    ...
    let mid = Math.floor((start + end) / 2);
    let leftQuery = this.query(root.left as SegmentTreeNode, L, R, start, mid);
    let rightQuery = this.query(root.right as SegmentTreeNode, L, R, mid + 1, end);
    return leftQuery + rightQuery;
  }
  ...
  queryRange(L: number, R: number, version: number): number {
    return this.query(this.roots[version], L, R, 0, this.n - 1);
  }
}
...
pst.updateValue(2, 10);  // Update index 2 to value 10
console.log(pst.queryRange(1, 3, 0));  // Query range [1, 3] in version 0
console.log(pst.queryRange(1, 3, 1));  // Query range [1, 3] in version 1
```
