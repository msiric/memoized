...
C -- 1 -- D
```
The MST would include edges: `A-B`, `C-D`, and either `A-C` or `B-D`, with a total weight of 5.
...
class BoruvkaMST {
    ...
    boruvkaMST(): { MSTweight: number, result: [number, number, number][] } {
        const uf = new UnionFind(this.V);
        const result: [number, number, number][] = [];
        let numTrees = this.V;
        let MSTweight = 0;
        ...
        while (numTrees > 1) {
            const cheapest: ([number, number, number] | null)[] = Array(this.V).fill(null);
