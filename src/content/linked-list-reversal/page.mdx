...
Consider a linked list: 1 -> 2 -> 3 -> 4 -> 5. After reversing, the linked list will be: 5 -> 4 -> 3 -> 2 -> 1.
...
function reverseBetween(head: ListNode | null, m: number, n: number): ListNode | null {
    ...
    const connection: ListNode | null = previous;
...
function reverseKGroup(head: ListNode | null, k: number): ListNode | null {
    ...
    let dummy = new ListNode(0);
    dummy.next = head;
    let current: ListNode | null = dummy, next: ListNode | null = dummy, previous: ListNode | null = dummy;
    let count = 0;
