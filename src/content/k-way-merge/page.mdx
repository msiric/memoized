...
Consider the sorted arrays `arr1 = [1, 4, 7]`, `arr2 = [2, 5, 8]`, and `arr3 = [3, 6, 9]`. The merged array is `[1, 2, 3, 4, 5, 6, 7, 8, 9]`.
...
```typescript
class MinHeap {
    heap: { value: number; arrayIndex: number; elementIndex: number }[];
    ...
    peek(): { value: number; arrayIndex: number; elementIndex: number } | undefined {
        return this.heap[0];
    }

    insert(value: { value: number; arrayIndex: number; elementIndex: number }): void {
        this.heap.push(value);
        this._heapifyUp();
    }

    extractMin(): { value: number; arrayIndex: number; elementIndex: number } | undefined {
        if (this.size() === 1) return this.heap.pop();
        const min = this.heap[0];
        this.heap[0] = this.heap.pop()!;
        this._heapifyDown();
...
const lists: ListNode[] = [list1, list2, list3];
const mergedList = mergeKLists(lists);
