...
Consider a list `[10, 7, 8, 9, 1, 5]` being sorted using randomized quick sort:
- A pivot is randomly chosen, elements are partitioned, and the process is recursively repeated.
- This randomization helps in achieving average-case time complexity and avoiding worst-case scenarios in many practical cases.
...
```typescript
function randomizedQuickSort(arr: number[], low: number = 0, high: number = arr.length - 1): void {
    if (low < high) {
        const pi = randomizedPartition(arr, low, high);
        randomizedQuickSort(arr, low, pi - 1);
        randomizedQuickSort(arr, pi + 1, high);
...
```typescript
function randomizedSelect(arr: number[], k: number, low: number = 0, high: number = arr.length - 1): number {
    if (low === high) return arr[low];
...
// Example usage:
const arr: number[] = [12, 3, 5, 7, 4, 19, 26];
const k: number = 3;
console.log(randomizedSelect(arr, k));  // Output: 5
```
...
```typescript
function isPrime(n: number, k: number = 5): boolean {
    if (n <= 1 || n === 4) return false;
    if (n <= 3) return true;
...
class RandomWalk {
    ...
    walk(start: number, steps: number): number {
        let current = start;
        for (let i = 0; i < steps; i++) {
            const neighbors = this.graph.getNeighbors(current);
            if (neighbors.length === 0) break;
