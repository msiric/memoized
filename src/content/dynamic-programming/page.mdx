...
function knapsack(values: number[], weights: number[], W: number): number {
    let n = values.length;
    let dp: number[][] = Array.from({ length: n + 1 }, () => Array(W + 1).fill(0));
...
function tsp(cost: number[][]): number {
    ...
    let dp: number[][] = Array.from({ length: 1 << n }, () => Array(n).fill(Infinity));
    dp[1][0] = 0;
...
function maxSum(root: TreeNode | null): number {
    function dfs(node: TreeNode | null): [number, number] {
        ...
        let left: [number, number] = dfs(node.left);
        let right: [number, number] = dfs(node.right);

        let includeNode: number = node.value + left[1] + right[1];
        let excludeNode: number = Math.max(left[0], left[1]) + Math.max(right[0], right[1]);

        return [includeNode, excludeNode];
    }

    let result: [number, number] = dfs(root);
    return Math.max(result[0], result[1]);
}
