...
Consider an array `nums = [3, 2, 1, 5, 6, 4]` and `K = 2`. The top 2 largest elements are `6` and `5`.
...
```typescript
class MinHeap {
    private heap: { num: number, freq: number }[];
    ...
    peek(): { num: number, freq: number } {
        return this.heap[0];
    }

    insert(value: { num: number, freq: number }): void {
        this.heap.push(value);
        this._heapifyUp();
    }

    extractMin(): { num: number, freq: number } {
        if (this.size() === 1) return this.heap.pop()!;
        const min = this.heap[0];
        this.heap[0] = this.heap.pop()!;
        this._heapifyDown();
