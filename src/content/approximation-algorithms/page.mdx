...
function vertexCover(graph: Graph): Set<number> {
  const cover = new Set<number>()
  const edges = new Set<string>(graph.edges.map((edge) => JSON.stringify(edge)))

  while (edges.size > 0) {
    const edge: Edge = JSON.parse([...edges][0])
    const [u, v] = edge
    ...
    for (let edgeStr of edges) {
      const edge = JSON.parse(edgeStr) as Edge
      if (edge.includes(u) || edge.includes(v)) {
...
function setCover(universe: number[], sets: Set<number>[]): Set<number>[] {
    ...
    const uncovered = new Set<number>(universe);
...
### **Approximation Algorithm for Metric Traveling Salesman Problem (TSP)**
...
class Graph {
    ...
    preorderTraversal(start: number, adjList: [number, number][][], visited: boolean[], result: number[]): void {
        visited[start] = true;
        result.push(start);
    ...
    metricTSP(): number[] {
        const mst = this.findMST();
        const adjList: [number, number][][] = Array.from({ length: this.V }, () => []);
...
```typescript
function knapsackFPTAS(values: number[], weights: number[], W: number, ε: number): number {
    const n = values.length;
    const K = Math.max(...values);
    const scale = ε * K / n;
    const scaledValues = values.map(v => Math.floor(v / scale));
