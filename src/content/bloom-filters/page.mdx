export const metadata = {
  title: 'Comprehensive Guide to Bloom Filters in JavaScript',
  description: 'Probabilistic data structures for efficient set membership testing. Understand bloom filters, performance considerations, and practical examples.',
}
...
Consider a Bloom Filter with a bit array of size 10 and 3 hash functions. When adding the element "hello":

1. Hash "hello" with hash functions h1, h2, and h3, which give indices 1, 3, and 7 respectively.
2. Set the bits at positions 1, 3, and 7 to 1 in the bit array.

Bit Array: [0, 1, 0, 1, 0, 0, 0, 1, 0, 0]

When checking for membership of "world":

1. Hash "world" with hash functions h1, h2, and h3, which give indices 2, 4, and 6 respectively.
2. Check the bits at positions 2, 4, and 6. If any of these bits are 0, "world" is definitely not in the set.

Bit Array: [0, 1, 0, 1, 0, 0, 0, 1, 0, 0] (Since positions 2 and 4 are 0, "world" is not in the set)
...
class BloomFilter {
    ...
    _hashFunction(value: string, seed: number): number {
        let hash = 0;
        for (let i = 0; i < value.length; i++) {
            hash = (hash * seed + value.charCodeAt(i)) % this.size;
        }
...
```typescript
function optimalBloomFilterParams(n: number, p: number): { m: number, k: number } {
    const m = Math.ceil(-n * Math.log(p) / (Math.log(2) ** 2));
    const k = Math.ceil((m / n) * Math.log(2));
    return { m, k };
}
...
class ScalableBloomFilter {
    ...
    addFilter(size: number, hashCount: number): void {
        this.filters.push(new BloomFilter(size, hashCount));
    }
