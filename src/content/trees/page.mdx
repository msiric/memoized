...
- **Trie:** A tree used to store strings, where each node represents a character.
...
Tries are tree-like data structures used to store strings, where each node represents a character.
...
console.log(bst.lowestCommonAncestor(bst.root, 5, 15));  // Output: 10
```
