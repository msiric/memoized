...
class BinarySearchTree<T> {
    ...
    inOrderTraverse(node: TreeNode<T> | null) {
        if (node !== null) {
            this.inOrderTraverse(node.left);
            console.log(node.value);
            this.inOrderTraverse(node.right);
...
class BinarySearchTree<T> {
    ...
    postOrderTraverse(node: TreeNode<T> | null) {
        if (node !== null) {
            this.postOrderTraverse(node.left);
            this.postOrderTraverse(node.right);
            console.log(node.value);
...
- **Use Recursion for Traversals:**
  ...
  **Example:**
  ...
  ```typescript
  function inOrder<T>(node: TreeNode<T> | null) {
      if (node !== null) {
          inOrder(node.left);
          console.log(node.value);
          inOrder(node.right);
...
- **Iterative Traversals:**
  ...
  **Example:**
  ...
  ```typescript
  function inOrderIterative<T>(root: TreeNode<T> | null) {
...
- **Balancing Trees:**
  ...
  **Example:**
  ...
  class AVLTree<T> {
      ...
      insertNode(node: TreeNode<T> | null, value: T): TreeNode<T> | null {
          if (node === null) {
...
- **Balancing Trees:**
  ...
  **Example:**
  ...
  ```typescript
  function balance(node: TreeNode<T>) {
      // Calculate balance factor and perform rotations
      if (this.getBalance(node) > 1) {
          // Perform rotations
      }
...
class AVLTree<T> {
    ...
    insertNode(node: AVLNode<T> | null, value: T): AVLNode<T> | null {
        if (node === null) {
    ...
    rotateLeft(z: AVLNode<T>): AVLNode<T> {
        const y = z.right!;
        const T2 = y.left;
    ...
    rotateRight(z: AVLNode<T>): AVLNode<T> {
        const y = z.left!;
        const T3 = y.right;
...
class RedBlackTree<T> {
    ...
    insertNode(node: RedBlackNode<T> | null, value: T): RedBlackNode<T> {
        if (node === null) return new RedBlackNode(value);
    ...
    flipColors(node: RedBlackNode<T>) {
        node.color = RED;
        node.left!.color = BLACK;
        node.right!.color = BLACK;
    }

    isRed(node: RedBlackNode<T> | null): boolean {
        if (node === null) return false;
        return node.color === RED;
    }
}
...
class BinarySearchTree<T> {
    ...
    lowestCommonAncestor(node: TreeNode<T> | null, p: T, q: T): TreeNode<T> | null {
        if (node === null) return null;
...
class BinaryTree<T> {
    ...
    serialize(root: TreeNode<T> | null): string {
        let result: string[] = [];
        this.serializeHelper(root, result);
        return result.join(',');
    }

    serializeHelper(node: TreeNode<T> | null, result: string[]): void {
        if (node === null) {
            result.push('#');
            return;
        }
    ...
    deserialize(data: string): TreeNode<T> | null {
        let values = data.split(',');
        let index = 0;

        const deserializeHelper = (): TreeNode<T> | null => {
            if (values[index] === '#') {
                index++;
                return null;
            }
            let node = new TreeNode(parseInt(values[index++]) as unknown as T);
            node.left = deserializeHelper();
            node.right = deserializeHelper();
            return node;
        };
