...
```typescript
let emptyMap: Map<unknown, unknown> = new Map();
let numberMap: Map<number, string> = new Map([[1, 'one'], [2, 'two'], [3, 'three']]);
let mixedMap: Map<unknown, unknown> = new Map([
    ['stringKey', 'value1'],
    [42, 'value2'],
    [{key: 'object'}, 'value3']
]);
...
```typescript
let map: Map<string, unknown> = new Map([['name', 'John'], ['age', 30]]);
console.log(map.has('name'));  // Output: true
console.log(map.has('address'));  // Output: false
```
...
```typescript
let map: Map<string, unknown> = new Map([['name', 'John'], ['age', 30]]);
let keysArray: string[] = Array.from(map.keys());
let valuesArray: unknown[] = Array.from(map.values());
let entriesArray: [string, unknown][] = Array.from(map.entries());
...
- **Using Maps for Object Metadata:**
  ...
  **Example:**
  ...
  let metadataMap: Map<object, { role: string }> = new Map();
  metadataMap.set(obj1, {role: 'admin'});
  metadataMap.set(obj2, {role: 'user'});
...
- **Using Complex Keys:**
  ...
  **Example:**
  ...
  ```typescript
  let map: Map<object | (() => void), string> = new Map();
  let key1 = {};
  let key2 = () => {};
...
- **Efficient Lookup Table:**
  ...
  **Example:**
  ...
  ```typescript
  let lookup: Map<string, number> = new Map([['a', 1], ['b', 2], ['c', 3]]);
  console.log(lookup.get('b'));  // Output: 2
  ```
...
- **Using Objects as Keys:**
  ...
  **Example:**
  ...
  ```typescript
  let map: Map<object, string> = new Map();
  let obj = {id: 1};
...
let map2: Map<string, number> = new Map([['c', 3], ['d', 4]]);

let mergedMap: Map<string, number> = mergeMaps(map1, map2);
console.log(mergedMap);  // Output: Map { 'a' => 1, 'b' => 2, 'c' => 3, 'd' => 4 }
...
function countFrequencies<T>(arr: T[]): Map<T, number> {
    let frequencyMap: Map<T, number> = new Map();
    for (let item of arr) {
        if (frequencyMap.has(item)) {
            frequencyMap.set(item, frequencyMap.get(item)! + 1);
        } else {
...
```typescript
function findUniqueKeys<T>(arr: T[]): T[] {
...
```typescript
function mergeMultipleMaps<K, V>(...maps: Map<K, V>[]): Map<K, V> {
    return maps.reduce((merged, map) => {
        for (let [key, value] of map) {
            merged.set(key, value);
        }
...
let map1: Map<string, number> = new Map([['a', 1], ['b', 2]]);
let map2: Map<string, number> = new Map([['c', 3], ['d', 4]]);
let map3: Map<string, number> = new Map([['e', 5], ['f', 6]]);

let mergedMap: Map<string, number> = mergeMultipleMaps(map1, map2, map3);
console.log(mergedMap);  // Output: Map { 'a' => 1, 'b' => 2, 'c' => 3, 'd' => 4, 'e' => 5, 'f' => 6 }
```
