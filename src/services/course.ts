...
export const getActiveCoursesWithProgress = async () => {
  ...
  const coursesWithProgress = activeCourses.map((course) => {
    const courseLessons = course.sections.flatMap(
    ...
    const completedProblems = userId
    ...
    return {
      ...
      metadata: {
        lessons: {
          total: totalLessons,
          free: courseLessons.filter(
            (lesson) => lesson.access === AccessOptions.FREE,
          ).length,
          premium: courseLessons.filter(
            (lesson) => lesson.access === AccessOptions.PREMIUM,
          ).length,
        },
        problems: {
          total: totalProblems,
...
export const getCoursesSlugs = async () => {
